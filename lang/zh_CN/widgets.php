<?php

return [
    'financial_stats' => [
        'receivables' => '应收款',
        'receivables_from_clients' => '应收款（来自客户）',
        'from_clients' => '来自客户',
        'payables' => '应付款',
        'payables_to_suppliers' => '应付款（给供应商）',
        'to_suppliers' => '给供应商',
        'payment_schedule' => '付款计划',
        'net_positive' => '净正值',
        'net_negative' => '净负值',
        'net_to_receive' => '净应收',
        'net_to_pay' => '净应付',
        'operational_expenses' => '运营费用',
        'vs_last_month' => '较上月',
    ],
    'cash_flow' => [
        'heading' => '现金流预测',
        'description' => '基于付款计划到期日的预计流入和流出',
        'period' => '期间',
        'inflow_pi' => '流入（PI）',
        'outflow_po' => '流出（PO）',
        'inflow' => '流入',
        'outflow' => '流出',
        'net' => '净额',
        'overdue' => '逾期',
        'before_today' => '今日之前',
        'this_week' => '本周',
        'next_week' => '下周',
        'no_due_date_warning' => '这些项目未设置到期日，未包含在上述预测中。',
    ],
    'document_summary' => [
        'financial_summary' => '财务摘要',
        'invoice_total' => '发票总额',
        'cost_margin' => '成本 / 利润',
        'fx_snapshot' => '汇率锁定',
        'margin' => '利润',
        'paid' => '已付款',
        'remaining' => '剩余',
        'credits' => '贷项',
        'total_due' => '应付总额',
        'net_due' => '净应付',
        'overdue' => '逾期',
        'next' => '下一个',
        'outstanding' => '未结清',
        'fully_paid' => '已全额付款',
        'waived' => '已豁免',
        'waived_description' => '免收 — 该期款项已豁免',
        'from_client_available' => '来自该客户 — 可分配',
        'to_supplier_available' => '给该供应商 — 可分配',
    ],
    'financial_statement' => [
        'client_receivables' => '客户报表 — 应收款',
        'supplier_payables' => '供应商报表 — 应付款',
        'forwarder_payables' => '货代报表 — 应付款',
        'overdue' => '逾期',
    ],
    'landed_cost' => [
        'heading' => '到岸成本计算器',
        'description' => '本次货运的完整成本分析',
        'freight' => '运费',
        'insurance' => '保险',
        'customs_duties' => '海关 / 关税',
        'inspection_testing' => '检验 / 测试',
        'packaging' => '包装',
        'other_costs' => '其他费用',
        'gross_profit' => '毛利润',
        'freight_margin' => '运费利润',
        'charged_to_client' => '向客户收取',
        'paid_to_forwarder' => '支付给货代',
        'freight_profit' => '运费利润',
        'client_amount' => '客户金额',
        'actual_cost' => '实际成本',
    ],
    'product_summary' => [
        'suppliers' => '供应商',
        'clients' => '客户',
        'variants' => '变体',
        'variant' => '变体',
        'base_price' => '基础价格',
        'manufacturing_cost' => '制造成本',
        'selling_price' => '销售价格',
        'markup' => '加价',
        'preferred_supplier' => '首选供应商',
        'preferred_client' => '首选客户',
        'weight' => '重量',
        'dimensions' => '尺寸',
        'material' => '材质',
        'color' => '颜色',
        'pcs_per_carton' => '每箱件数',
        'lead_time' => '交货期',
    ],
    'pipeline' => [
        'operations_pipeline' => '运营管道',
    ],
    'alerts' => [
        'action_required' => '需要操作',
        'all_clear' => '一切正常 — 无待处理事项。',
        'overdue_payables_desc' => '应付账款已逾期且未全额支付',
        'overdue_receivables_desc' => '应收账款已逾期且未全额收款',
        'view_payables' => '查看应付账款',
        'view_receivables' => '查看应收账款',
        'pending_approval_payables_desc' => '已提交的供应商付款，等待审核',
        'pending_approval_receivables_desc' => '已提交的客户收款，等待审核',
        'review_payables' => '审核应付账款',
        'review_receivables' => '审核应收账款',
        'finalized_pi_desc' => '形式发票已确认，但尚未创建采购订单',
        'view_pis' => '查看PI',
        'stalled_po_desc' => '生产中的采购订单超过15天无活动',
        'view_pos' => '查看PO',
        'open_inquiries_desc' => '可能需要跟进的客户询价',
        'view_inquiries' => '查看询价',
        'due_this_week_payables_desc' => '未来7天内到期的应付账款',
        'due_this_week_receivables_desc' => '未来7天内到期的应收账款',
        'eta_changed_title' => '货运 :ref 的 ETA 已变更为 :eta',
        'eta_changed_desc' => '货代 :forwarder 更新了 ETA。请重新计算付款计划。',
        'view_shipment' => '查看货运',
        'recalc_payment_schedule' => '重新计算',
        'recalc_done_title' => '付款计划已重新计算',
        'recalc_done_body' => '货运 :ref 的 :count 个付款计划项已重新生成。',
        'recalc_unauthorized' => '只有货运负责人可以重新计算。',
        'stale_schedule_title' => '付款计划已过期：:ref',
        'stale_schedule_desc' => '计划金额为 :scheduled，但当前值合计为 :expected。请重新生成付款计划。',
        'view_document' => '查看单据',
        'regenerate_schedule' => '重新生成',
        'regenerate_unauthorized' => '您没有权限重新生成付款计划。',
        'regenerate_done_title' => '付款计划已重新生成',
        'regenerate_done_body' => ':ref 的 :count 个付款计划项已重新生成。',
    ],
    'audit_stats' => [
        'scheduled_audits' => '计划审核',
        'in_progress' => '进行中',
        'pending_audits' => '待审核',
        'completed_this_month' => '本月完成',
        'average_score' => '平均得分',
        'overdue' => '逾期',
        'rejected_ytd' => '年初至今拒绝',
    ],
    'kanban' => [
        'value' => '价值',
        'paid' => '已付款',
        'overdue_payment' => '逾期付款',
        'no_update_for' => '无更新',
        'no_items' => '无项目',
    ],
    'expenses' => [
        'entries' => '条记录',
        'missing_exchange_rate' => '缺少汇率 — 未计入总计',
        'year_to_date' => '年度累计',
        'monthly_average' => '月均',
        'category_breakdown' => '分类明细',
    ],

    'recurring' => [
        'heading' => '定期费用',
        'pending' => '待处理',
        'paid' => '已付款',
        'due_day' => '第:day日',
        'register_payment' => '登记付款',
        'payment_registered' => '付款登记成功！',
        'no_recurring' => '没有登记的定期费用。',
    ],

    // Portal List Stats Widgets
    'list_stats' => [
        'total_shipments' => '总发货量',
        'active' => '活跃',
        'in_transit' => '运输中',
        'arrived' => '已到达',
        'status_breakdown' => '状态明细',
        'total_quotations' => '报价单总数',
        'pending' => '待处理',
        'approved' => '已批准',
        'rejected' => '已拒绝',
        'total_value' => '总价值',
        'approved_value' => '已批准价值',
        'total_invoices' => '发票总数',
        'confirmed' => '已确认',
        'finalized' => '已完成',
        'payment_progress' => '付款进度',
        'paid' => '已付款',
        'remaining' => '剩余',
        'total_payments' => '付款总数',
        'allocated' => '已分配',
        'unallocated' => '未分配',
        'applied_to_invoices' => '已应用于发票',
        'pending_allocation' => '待分配',
        'fully_allocated' => '已全额分配',
    ],

    // Portal Upcoming Arrivals Widget
    'arrivals' => [
        'upcoming_arrivals' => '即将到达',
        'expected_payments' => '预计付款',
        'this_week' => '本周',
        'week_2' => '第2周',
        'week_3' => '第3周',
        'week_4' => '第4周',
        'week' => '周',
        'reference' => '参考',
        'status' => '状态',
        'transport' => '运输方式',
        'container' => '集装箱',
        'eta' => '预计到达',
        'days_until' => '天数',
        'destination' => '目的地',
        'today' => '今天',
        'late' => '延迟',
        'no_upcoming' => '未来4周没有预计到达的货物。',
    ],

    // Portal Detail Widgets
    'portal' => [
        'active_shipments' => '活跃发货',
        'no_active_shipments' => '目前没有活跃的发货。',
        'recent_eta_changes' => 'ETA 变更通知',
        'recent_eta_changes_desc' => '过去 14 天内货代更新了下列货运的 ETA。',
        'eta_changed_for' => '货运 :ref 的 ETA 已变更',
        'eta_now' => '新 ETA：:eta',
        'changed_at' => '更新时间：:date',
        'status' => '状态',
        'eta' => '预计到达',
        'etd' => '预计出发',
        'transport' => '运输方式',
        'carrier' => '承运人',
        'vessel' => '船舶',
        'container' => '集装箱',
        'origin_port' => '起运港',
        'destination_port' => '目的港',
        'booking_number' => '订舱号',
        'bl_number' => '提单号',
        'items' => '项目',
        'total_value' => '总价值',
        'product_value' => '产品价值',
        'currency' => '货币',
        'total_pi_value' => 'PI总价值',
        'total_paid' => '已付总额',
        'pending_balance' => '待付余额',
        'payments_received' => '已收付款',
        'outstanding_payments' => '未付款项',
        'confirmed_proforma_invoices' => '已确认形式发票',
        'requires_attention' => '需要关注',
        'recent_documents' => '最近文档',
        'no_documents_yet' => '暂无可用文档。',
        'commercial_invoice' => '商业发票',
        'packing_list' => '装箱单',
        'generated' => '已生成',
        'type' => '类型',
        'reference' => '参考编号',
        'received' => '已收到',
        'logistics_details' => '物流详情',
        'packages' => '包裹',
        'gross_weight' => '毛重',
        'volume' => '体积',
        'shipping_timeline' => '运输时间线',
        'estimated' => '预计',
        'actual' => '实际',
        'documents' => '文档',
        'download' => '下载',
        'no_documents_available' => '暂无可用文档。',
        'payment_allocations' => '付款分配',
        'payment_amount' => '付款金额',
        'document' => '文档',
        'schedule_item' => '计划项目',
        'allocated_amount' => '分配金额',
        'total' => '总计',
        'allocation' => '项分配',
        'allocations' => '项分配',
        'no_allocations_yet' => '尚无分配记录。',
    ],

    // Shipment Fulfillment Widget
    'fulfillment' => [
        'shipment_fulfillment' => '发货履行',
        'fulfillment_progress' => '履行进度',
        'total_items' => '总项目数',
        'fully_shipped' => '已全部发货',
        'partial_shipments' => '部分发货',
        'items_pending' => '待发货项目',
        'items_partially_shipped' => '部分发货项目',
        'units_shipped' => '已发货数量',
        'units_total' => '总数量',
        'units' => '单位',
        'remaining' => '剩余',
        'product' => '产品',
        'pi_qty' => 'PI数量',
        'shipped' => '已发货',
        'status' => '状态',
        'shipments' => '发货',
        'partial' => '部分',
        'pending_status' => '待处理',
        'item' => '项',
        'items' => '项',
        'no_items' => '此形式发票中没有项目。',
        'ready_for_finalization' => '所有项目已发货 — 可以完成',
        'fulfillment_incomplete' => '发货履行未完成',
        'units_pending_shipment' => ':units 个单位仍在 :items 个项目中等待发货',
    ],

    // My Projects Widget
    'my_projects' => [
        'heading' => '我的项目',
        'active_items' => '分配给您的活跃项目',
        'inquiries' => '询价',
        'quotations' => '报价',
        'supplier_quotations' => '供应商报价',
        'proforma_invoices' => '形式发票',
        'purchase_orders' => '采购订单',
        'shipments' => '发货',
        'needs_attention' => '需要您的关注',
        'recent_inquiries' => '您最近的询价',
        'reference' => '参考',
        'client' => '客户',
        'status' => '状态',
        'updated' => '已更新',
        'no_projects_assigned' => '尚未分配任何项目给您。',
        'inquiries_no_update_5_days' => '5天以上未更新的询价',
        'pos_no_update_10_days' => '10天以上未更新的采购订单',
        'finalized_pis_without_po' => '已完成但未创建采购订单的形式发票',
    ],

    // Shipment Plan Summary Widget
    'shipment_plan' => [
        'total_planned_value' => '计划总金额',
        'items_from_pis' => '来自 :pis 张形式发票的 :items 个项目',
        'total_paid' => '已付总额',
        'remaining' => '剩余 :amount',
        'blocking_payments' => '阻止发货的付款',
        'must_be_paid_before_shipping' => '发货前必须付清',
        'all_clear' => '全部完成',
    ],

    // Financial Dashboard Widgets
    'financial_dashboard' => [
        'receivables' => '应收（未结）',
        'payables' => '应付（未结）',
        'overdue' => '逾期',
        'net_open' => '未结净额',
        'open_items_count' => ':count 笔未结分期',
        'cash_flow_heading' => '现金流 — 未来6个月',
        'cash_flow_inflow' => '流入',
        'cash_flow_outflow' => '流出',
        'cash_flow_net' => '净额',
        'aging_receivables_heading' => '应收账龄',
        'aging_payables_heading' => '应付账龄',
        'aging_current' => '未到期',
        'aging_1_30' => '1–30天',
        'aging_31_60' => '31–60天',
        'aging_60_plus' => '60天以上',
        'upcoming_receivables_heading' => '即将到期的应收',
        'upcoming_payables_heading' => '即将到期的付款',
        'top_debtors_heading' => '未结余额最高的客户',
        'top_debtors_empty' => '没有未结应收款。',
        'currency_exposure_heading' => '按币种的未结余额',
        'conversion_warning' => ':codes 金额无法折算（缺少汇率）。',
    ],
];
