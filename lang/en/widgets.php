<?php

return [
    'financial_stats' => [
        'receivables' => 'Receivables',
        'receivables_from_clients' => 'Receivables (from Clients)',
        'from_clients' => 'From clients',
        'payables' => 'Payables',
        'payables_to_suppliers' => 'Payables (to Suppliers)',
        'to_suppliers' => 'To suppliers',
        'payment_schedule' => 'Payment Schedule',
        'net_positive' => 'Net Positive',
        'net_negative' => 'Net Negative',
        'net_to_receive' => 'Net to Receive',
        'net_to_pay' => 'Net to Pay',
        'operational_expenses' => 'Operational Expenses',
        'vs_last_month' => 'vs last month',
    ],
    'cash_flow' => [
        'heading' => 'Cash Flow Projection',
        'description' => 'Expected inflows and outflows based on payment schedule due dates',
        'period' => 'Period',
        'inflow_pi' => 'Inflow (PI)',
        'outflow_po' => 'Outflow (PO)',
        'inflow' => 'Inflow',
        'outflow' => 'Outflow',
        'net' => 'Net',
        'overdue' => 'Overdue',
        'before_today' => 'Before today',
        'this_week' => 'This Week',
        'next_week' => 'Next Week',
        'no_due_date_warning' => 'These items have no due date set and are not included in the projection above.',
    ],
    'document_summary' => [
        'financial_summary' => 'Financial Summary',
        'invoice_total' => 'Invoice Total',
        'cost_margin' => 'Cost / Margin',
        'fx_snapshot' => 'FX snapshot',
        'margin' => 'Margin',
        'paid' => 'Paid',
        'remaining' => 'Remaining',
        'credits' => 'Credits',
        'total_due' => 'Total Due',
        'net_due' => 'Net Due',
        'overdue' => 'Overdue',
        'next' => 'Next',
        'outstanding' => 'Outstanding',
        'fully_paid' => 'Fully paid',
        'waived' => 'Waived',
        'waived_description' => 'No charge — this installment was waived',
        'from_client_available' => 'From this client — available to allocate',
        'to_supplier_available' => 'To this supplier — available to allocate',
    ],
    'financial_statement' => [
        'client_receivables' => 'Client Statement — Receivables',
        'supplier_payables' => 'Supplier Statement — Payables',
        'forwarder_payables' => 'Forwarder Statement — Payables',
        'overdue' => 'Overdue',
    ],
    'landed_cost' => [
        'heading' => 'Landed Cost Calculator',
        'description' => 'Complete cost analysis for this shipment',
        'freight' => 'Freight',
        'insurance' => 'Insurance',
        'customs_duties' => 'Customs / Duties',
        'inspection_testing' => 'Inspection / Testing',
        'packaging' => 'Packaging',
        'other_costs' => 'Other Costs',
        'gross_profit' => 'Gross Profit',
        'freight_margin' => 'Freight Margin',
        'charged_to_client' => 'Charged to Client',
        'paid_to_forwarder' => 'Paid to Forwarder',
        'freight_profit' => 'Freight Profit',
        'client_amount' => 'Client Amount',
        'actual_cost' => 'Actual Cost',
    ],
    'product_summary' => [
        'suppliers' => 'Suppliers',
        'clients' => 'Clients',
        'variants' => 'Variants',
        'variant' => 'Variant',
        'base_price' => 'Base Price',
        'manufacturing_cost' => 'Manufacturing Cost',
        'selling_price' => 'Selling Price',
        'markup' => 'Markup',
        'preferred_supplier' => 'Preferred Supplier',
        'preferred_client' => 'Preferred Client',
        'weight' => 'Weight',
        'dimensions' => 'Dimensions',
        'material' => 'Material',
        'color' => 'Color',
        'pcs_per_carton' => 'Pcs/Carton',
        'lead_time' => 'Lead Time',
    ],
    'pipeline' => [
        'operations_pipeline' => 'Operations Pipeline',
    ],
    'alerts' => [
        'action_required' => 'Action Required',
        'all_clear' => 'All clear — no pending actions.',
        'overdue_payables_desc' => 'Accounts payable past due date without full payment',
        'overdue_receivables_desc' => 'Accounts receivable past due date without full receipt',
        'view_payables' => 'View Payables',
        'view_receivables' => 'View Receivables',
        'pending_approval_payables_desc' => 'Supplier payments submitted and waiting for approval',
        'pending_approval_receivables_desc' => 'Client receipts submitted and waiting for approval',
        'review_payables' => 'Review Payables',
        'review_receivables' => 'Review Receivables',
        'finalized_pi_desc' => 'Proforma invoices confirmed but no purchase order created yet',
        'view_pis' => 'View PIs',
        'stalled_po_desc' => 'Purchase orders in production with no activity for over 15 days',
        'view_pos' => 'View POs',
        'open_inquiries_desc' => 'Client inquiries that may need follow-up',
        'view_inquiries' => 'View Inquiries',
        'due_this_week_payables_desc' => 'Accounts payable due within the next 7 days',
        'due_this_week_receivables_desc' => 'Accounts receivable due within the next 7 days',
        'eta_changed_title' => 'Shipment :ref ETA changed to :eta',
        'eta_changed_desc' => 'Forwarder :forwarder updated the ETA. Recalculate the payment schedule.',
        'view_shipment' => 'View Shipment',
        'recalc_payment_schedule' => 'Recalculate',
        'recalc_done_title' => 'Schedule recalculated',
        'recalc_done_body' => ':count schedule items for shipment :ref were regenerated.',
        'recalc_unauthorized' => 'Only the shipment owner may recalculate.',
        'stale_schedule_title' => 'Payment schedule out of date: :ref',
        'stale_schedule_desc' => 'Scheduled :scheduled but current values total :expected. Regenerate the payment schedule.',
        'view_document' => 'View Document',
        'regenerate_schedule' => 'Regenerate',
        'regenerate_unauthorized' => 'You are not allowed to regenerate payment schedules.',
        'regenerate_done_title' => 'Schedule regenerated',
        'regenerate_done_body' => ':count schedule items for :ref were regenerated.',
    ],
    'audit_stats' => [
        'scheduled_audits' => 'Scheduled Audits',
        'in_progress' => 'in progress',
        'pending_audits' => 'Pending audits',
        'completed_this_month' => 'Completed This Month',
        'average_score' => 'Average Score',
        'overdue' => 'Overdue',
        'rejected_ytd' => 'Rejected (YTD)',
    ],
    'kanban' => [
        'value' => 'Value',
        'paid' => 'Paid',
        'overdue_payment' => 'Overdue payment',
        'no_update_for' => 'No update for',
        'no_items' => 'No items',
    ],
    'expenses' => [
        'entries' => 'entries',
        'missing_exchange_rate' => 'Missing exchange rate — not included in total',
        'year_to_date' => 'Year to Date',
        'monthly_average' => 'Monthly Average',
        'category_breakdown' => 'Category Breakdown',
    ],

    'recurring' => [
        'heading' => 'Recurring Expenses',
        'pending' => 'pending',
        'paid' => 'paid',
        'due_day' => 'Day :day',
        'register_payment' => 'Register Payment',
        'payment_registered' => 'Payment registered successfully!',
        'no_recurring' => 'No recurring expenses registered.',
    ],

    // Portal List Stats Widgets
    'list_stats' => [
        'total_shipments' => 'Total Shipments',
        'active' => 'Active',
        'in_transit' => 'In Transit',
        'arrived' => 'Arrived',
        'status_breakdown' => 'Status Breakdown',
        'total_quotations' => 'Total Quotations',
        'pending' => 'Pending',
        'approved' => 'Approved',
        'rejected' => 'Rejected',
        'total_value' => 'Total Value',
        'approved_value' => 'Approved Value',
        'total_invoices' => 'Total Invoices',
        'confirmed' => 'Confirmed',
        'finalized' => 'Finalized',
        'payment_progress' => 'Payment Progress',
        'paid' => 'Paid',
        'remaining' => 'Remaining',
        'total_payments' => 'Total Payments',
        'allocated' => 'Allocated',
        'unallocated' => 'Unallocated',
        'applied_to_invoices' => 'Applied to invoices',
        'pending_allocation' => 'Pending allocation',
        'fully_allocated' => 'Fully allocated',
    ],

    // Portal Upcoming Arrivals Widget
    'arrivals' => [
        'upcoming_arrivals' => 'Upcoming Arrivals',
        'expected_payments' => 'Expected payments',
        'this_week' => 'This Week',
        'week_2' => 'Week 2',
        'week_3' => 'Week 3',
        'week_4' => 'Week 4',
        'week' => 'Week',
        'reference' => 'Reference',
        'status' => 'Status',
        'transport' => 'Transport',
        'container' => 'Container',
        'eta' => 'ETA',
        'days_until' => 'Days',
        'destination' => 'Destination',
        'today' => 'Today',
        'late' => 'late',
        'no_upcoming' => 'No upcoming arrivals in the next 4 weeks.',
    ],

    // Portal Detail Widgets
    'portal' => [
        'active_shipments' => 'Active Shipments',
        'no_active_shipments' => 'No active shipments at this time.',
        'recent_eta_changes' => 'ETA Change Notices',
        'recent_eta_changes_desc' => 'The forwarder updated the ETA on the shipments below in the last 14 days.',
        'eta_changed_for' => 'ETA changed for shipment :ref',
        'eta_now' => 'New ETA: :eta',
        'changed_at' => 'Updated on :date',
        'status' => 'Status',
        'eta' => 'ETA',
        'etd' => 'ETD',
        'transport' => 'Transport',
        'carrier' => 'Carrier',
        'vessel' => 'Vessel',
        'container' => 'Container',
        'origin_port' => 'Origin Port',
        'destination_port' => 'Destination Port',
        'booking_number' => 'Booking Number',
        'bl_number' => 'B/L Number',
        'items' => 'Items',
        'total_value' => 'Total Value',
        'product_value' => 'Product Value',
        'currency' => 'Currency',
        'total_pi_value' => 'Total PI Value',
        'total_paid' => 'Total Paid',
        'pending_balance' => 'Pending Balance',
        'payments_received' => 'Payments Received',
        'outstanding_payments' => 'Outstanding Payments',
        'confirmed_proforma_invoices' => 'Confirmed Proforma Invoices',
        'requires_attention' => 'Requires Attention',
        'recent_documents' => 'Recent Documents',
        'no_documents_yet' => 'No documents available yet.',
        'commercial_invoice' => 'Commercial Invoice',
        'packing_list' => 'Packing List',
        'generated' => 'Generated',
        'type' => 'Type',
        'reference' => 'Reference',
        'received' => 'Received',
        'logistics_details' => 'Logistics Details',
        'packages' => 'Packages',
        'gross_weight' => 'Gross Weight',
        'volume' => 'Volume',
        'shipping_timeline' => 'Shipping Timeline',
        'estimated' => 'Estimated',
        'actual' => 'Actual',
        'documents' => 'Documents',
        'download' => 'Download',
        'no_documents_available' => 'No documents available yet.',
        'payment_allocations' => 'Payment Allocations',
        'payment_amount' => 'Payment Amount',
        'document' => 'Document',
        'schedule_item' => 'Schedule Item',
        'allocated_amount' => 'Allocated Amount',
        'total' => 'Total',
        'allocation' => 'allocation',
        'allocations' => 'allocations',
        'no_allocations_yet' => 'No allocations recorded yet.',
    ],

    // Shipment Fulfillment Widget
    'fulfillment' => [
        'shipment_fulfillment' => 'Shipment Fulfillment',
        'fulfillment_progress' => 'Fulfillment Progress',
        'total_items' => 'Total Items',
        'fully_shipped' => 'Fully Shipped',
        'partial_shipments' => 'Partial Shipments',
        'items_pending' => 'Items Pending',
        'items_partially_shipped' => 'Items Partially Shipped',
        'units_shipped' => 'Units Shipped',
        'units_total' => 'Units Total',
        'units' => 'units',
        'remaining' => 'Remaining',
        'product' => 'Product',
        'pi_qty' => 'PI Qty',
        'shipped' => 'Shipped',
        'in_shipment' => 'In Shipment',
        'in_shipment_hint' => 'Booked or at customs, not departed yet — not counted as shipped',
        'status' => 'Status',
        'shipments' => 'Shipments',
        'partial' => 'Partial',
        'pending_status' => 'Pending',
        'item' => 'item',
        'items' => 'items',
        'no_items' => 'No items in this Proforma Invoice.',
        'ready_for_finalization' => 'All items shipped — ready for finalization',
        'fulfillment_incomplete' => 'Shipment fulfillment incomplete',
        'units_pending_shipment' => ':units unit(s) still pending shipment across :items item(s)',
    ],

    // My Projects Widget
    'my_projects' => [
        'heading' => 'My Projects',
        'active_items' => 'active items assigned to you',
        'inquiries' => 'Inquiries',
        'quotations' => 'Quotations',
        'supplier_quotations' => 'Supplier Quotes',
        'proforma_invoices' => 'Proforma Invoices',
        'purchase_orders' => 'Purchase Orders',
        'shipments' => 'Shipments',
        'needs_attention' => 'Needs Your Attention',
        'recent_inquiries' => 'Your Recent Inquiries',
        'reference' => 'Reference',
        'client' => 'Client',
        'status' => 'Status',
        'updated' => 'Updated',
        'no_projects_assigned' => 'No projects assigned to you yet.',
        'inquiries_no_update_5_days' => 'inquiry(ies) with no update for 5+ days',
        'pos_no_update_10_days' => 'PO(s) with no update for 10+ days',
        'finalized_pis_without_po' => 'finalized PI(s) without Purchase Order',
    ],

    // Shipment Plan Summary Widget
    'shipment_plan' => [
        'total_planned_value' => 'Total Planned Value',
        'items_from_pis' => ':items items from :pis PI(s)',
        'total_paid' => 'Total Paid',
        'remaining' => ':amount remaining',
        'blocking_payments' => 'Blocking Payments',
        'must_be_paid_before_shipping' => 'Must be paid before shipping',
        'all_clear' => 'All clear',
    ],

    // Financial Dashboard Widgets
    'financial_dashboard' => [
        'receivables' => 'Receivables (open)',
        'payables' => 'Payables (open)',
        'overdue' => 'Overdue',
        'net_open' => 'Net open balance',
        'open_items_count' => '{1} :count open installment|[2,*] :count open installments',
        'cash_flow_heading' => 'Cash flow — next 6 months',
        'cash_flow_inflow' => 'Inflows',
        'cash_flow_outflow' => 'Outflows',
        'cash_flow_net' => 'Net',
        'aging_receivables_heading' => 'Receivables aging',
        'aging_payables_heading' => 'Payables aging',
        'aging_current' => 'Not due',
        'aging_1_30' => '1–30 days',
        'aging_31_60' => '31–60 days',
        'aging_60_plus' => '60+ days',
        'upcoming_receivables_heading' => 'Upcoming receivables',
        'upcoming_payables_heading' => 'Upcoming payments',
        'top_debtors_heading' => 'Top clients by open balance',
        'top_debtors_empty' => 'No open receivables.',
        'currency_exposure_heading' => 'Open balance by currency',
        'conversion_warning' => 'Amounts in :codes could not be converted (missing exchange rate).',
    ],
];
