    ...
    public function build(Company $company, FinancialReportFilters $filters): StatementSection
    {
        ...
        $rows = $query->with(['items', 'additionalCosts'])
            ...
            ->map(function (ProformaInvoice $pi) {
                ...
                $totalRevenue = $revenue + $commission;
                $marginAmount = $totalRevenue - $cost;
                ...
                return [
                    ...
                    'total_revenue' => round($totalRevenue / Money::SCALE, 2),
        ...
        return new StatementSection(
            ...
            columns: ['pi_reference', 'revenue', 'commission', 'total_revenue', 'cost', 'margin_amount', 'margin_pct', 'currency'],
            rows: $rows,
        );
    }
}
