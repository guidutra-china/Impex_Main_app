            ...
            Section::make(__('forms.sections.product_identity'))
                ->schema([
                    Select::make('category_id')
                        ...
                        ->afterStateUpdated(function (Set $set, ?string $state, Get $get, $livewire) {
                            // Only auto-populate SKU on create — never overwrite on edit
                    ...
                    TextInput::make('name')
                        ...
                        ->required()
                        ->maxLength(255)
                        ->helperText(__('forms.helpers.click_arrow_to_generate_from_category_model'))
                        ->suffixAction(
                            \Filament\Actions\Action::make('generateName')
                    ...
                    TextInput::make('product_family')
                        ...
                        ->maxLength(255)
                        ->helperText(__('forms.helpers.product_family_group'))
                        ->placeholder(__('forms.placeholders.product_family'))
                    ...
                    TextInput::make('sku')
                        ...
                        ->suffixAction(
                            \Filament\Actions\Action::make('regenerateSku')
                                ->icon('heroicon-m-arrow-path')
                                ->tooltip(__('forms.tooltips.regenerate_sku'))
                                ->visible(fn ($livewire) => $livewire instanceof \Filament\Resources\Pages\EditRecord)
                                ->requiresConfirmation()
                                ->modalHeading(__('forms.modals.regenerate_sku_heading'))
                                ->modalDescription(__('forms.modals.regenerate_sku_description'))
                                ->action(function (Set $set, Get $get) {
                                    $categoryId = $get('category_id');
                                    if ($categoryId) {
                                        $set('sku', app(GenerateProductSkuAction::class)->execute((int) $categoryId));
                                    } else {
                                        $set('sku', app(GenerateProductSkuAction::class)->generateDraftSku());
                                    }
                                })
                        ),
                    Select::make('status')
