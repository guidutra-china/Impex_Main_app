            ...
            Section::make(__('forms.sections.credit_note_details'))
                ...
                ->schema([
                    ...
                    TextEntry::make('applied_amount')
                        ...
                        ->state(fn ($record) => $record->applied_amount)
                        ->formatStateUsing(fn ($state) => Money::format((int) $state))
                        ->badge()
                        ->color(fn ($state) => ((int) $state) > 0 ? 'success' : 'gray'),
                    TextEntry::make('refunded_amount')
                        ->label(__('forms.labels.refunded_amount'))
                        ->state(fn ($record) => $record->refunded_amount)
                        ->formatStateUsing(fn ($state) => Money::format((int) $state))
                        ->badge()
                        ->color(fn ($state) => ((int) $state) > 0 ? 'success' : 'gray'),
                        ...
                        ->schema([
                            ...
                            TextEntry::make('document_reference')
                                ...
                                ->getStateUsing(function ($record) {
                                    return $record->proformaInvoice?->reference
                                        ?? $record->purchaseOrder?->reference
