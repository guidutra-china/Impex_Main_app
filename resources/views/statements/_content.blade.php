            ...
            @if(count($moneyCols) > 0)
                ...
                <tfoot>
                    @foreach(collect($section->rows)->groupBy(fn ($r) => (string) ($r['currency'] ?? '')) as $cur => $curRows)
                        <tr style="font-weight: 700; background: #f3f4f6; border-top: 2px solid #9ca3af;">
                            @foreach($section->columns as $col)
                                @if($loop->first)
                                    <td style="padding: 6px 8px; font-size: 12px;">{{ __('statements.columns.total') }} {{ $cur }}</td>
                                @elseif(in_array($col, $moneyCols))
                                    <td style="text-align: right; padding: 6px 8px; font-size: 12px;">{{ number_format($curRows->sum(fn ($r) => (float) ($r[$col] ?? 0)), 2) }}</td>
                                @elseif($col === 'currency')
                                    <td style="padding: 6px 8px; font-size: 12px;">{{ $cur }}</td>
                                @else
                                    <td></td>
                                @endif
                            @endforeach
