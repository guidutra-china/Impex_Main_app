...
//
// The actual sync logic lives in the SPA (resources/js/trips-mobile/sync.js)
...
const VERSION = 'trips-mobile-v1';
const SHELL_CACHE = `${VERSION}-shell`;
const SYNC_TAG = 'trips-mobile-sync';

const SHELL_URLS = [
    '/trips-mobile',
    '/trips-mobile/manifest.json',
...
self.addEventListener('fetch', (event) => {
    ...
    if (
        url.pathname.startsWith('/trips-mobile') ||
...
async function staleWhileRevalidate(request) {
    ...
    if (request.mode === 'navigate') {
        const shell = await cache.match('/trips-mobile');
